...
echo -e "\n${YELLOW}[1/7] Run identifier${NC}"
echo "  Output goes to results/<run_id>/. Use a short, descriptive name."
ask RUN_ID "Run ID (e.g. study1, pilot_run)" "my_run"

# ── output_dir ────────────────────────────────────────────────────────────────

echo -e "\n${YELLOW}[2/7] Output base directory${NC}"
echo "  All pipeline outputs (intermediates + final results) go to <output_dir>/<run_id>/."
echo "  Use a relative path (e.g. 'results') or absolute path (e.g. '/data/myproject')."
ask OUTPUT_DIR "Output base directory" "results"

# ── samples.tsv ──────────────────────────────────────────────────────────────

echo -e "\n${YELLOW}[3/7] Samples file${NC}"
...
echo -e "\n${YELLOW}[4/7] SILVA reference (silva.nr_v132.align)${NC}"
...
echo -e "\n${YELLOW}[5/7] USEARCH 8.1.1861${NC}"
...
echo -e "\n${YELLOW}[7/7] Advanced parameters${NC}"
